...
3. شغّل سكربت SQL الجاهز في ملف `sql/users_schema_fix.sql` من خلال **SQL Editor** في لوحة Supabase (الصورة التوضيحية في طلبك هي القائمة المطلوبة). السكربت يقوم بضبط الأنواع والربط مع `auth.users`، يضيف قيود فريدة على البريد، يفعل RLS، وينشئ السياسات اللازمة مع حماية من التكرار.
